<?php
/**
 * Date/Time Recurrence Minutely Frequency Class
...
class Minutely extends \qCal\DateTime\Recur\Freq {

    public function getNextInterval(\qCal\DateTime $date) {
    
        return DT::rollover($date->getYear(), $date->getMonth(), $date->getMonthDay(), $date->getHour(), $date->getMinute() + $this->getInterval(), $date->getSecond());
    ...
    public function getRecurrences($start) {
        ...
        foreach ($rules['bySecond'] as $second) {
            $dt = DT::rollover($start->getYear(), $start->getMonth(), $start->getMonthDay(), $start->getHour(), $start->getMinute(), $second);

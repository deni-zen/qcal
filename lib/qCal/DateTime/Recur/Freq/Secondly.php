<?php
/**
 * Date/Time Recurrence Secondly Frequency Class
...
class Secondly extends \qCal\DateTime\Recur\Freq {

    public function getNextInterval(\qCal\DateTime $date) {
    
        return DT::rollover($date->getYear(), $date->getMonth(), $date->getMonthDay(), $date->getHour(), $date->getMinute(), $date->getSecond() + $this->getInterval());
    ...
    public function getRecurrences($start) {
        ...
        if (!empty($rules['byMonth'])) {
            ...
            if (in_array($start->getMonth(), $months)) {
                $datevalid['byMonth'] = true;
                if (!empty($rules['byMonthDay'])) {
                    ...
                    foreach ($rules['byMonthDay'] as $monthDay) {
                        ...
                        if ($monthDay < 0) {
                            // @todo I had to make this add a day to get -1 to result in the 31st
                            // it really doesn't matter though because when I refactor, this will
                            // be moved to a method and it will always calculate correctly
                            $monthDay = $start->getDaysInMonth() + 1 + $monthDay;
                        }
        ...
        // if minute doesn't fall within one of the specified minutes, go no further
        if (!empty($rules['byMinute'])) {
            if (!in_array($start->getMinute(), $rules['byMinute'])) {
                $datevalid['byMinute'] = false;
            }
        }
        // if second doesn't fall within one of the specified seconds, go no further
        if (!empty($rules['bySecond'])) {
            if (!in_array($start->getSecond(), $rules['bySecond'])) {
                $datevalid['bySecond'] = false;
            }
        }
        ...
        $dt = DT::rollover($start->getYear(), $start->getMonth(), $start->getMonthDay(), $start->getHour(), $start->getMinute(), $start->getSecond());
